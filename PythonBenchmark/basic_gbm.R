...
num_train <- num_validate <- num_test <- 1000000
num_stats <- 0
num_submission <- dim(test)[1]

data <- split_data(data, num_train, num_validate, num_stats, num_test, num_submission)
...
save(gbm.ndcg, file="../Models/gbm.ndcg.basic.RData")
...
write.table(submission_p, "../Submissions/submssision_gbm.basic.csv", sep = ",", row.names=FALSE, quote=FALSE)
